...
Champion selection compares exactly three seeds (17, 42, and 2026). Paired bootstrap intervals are descriptive uncertainty for those repeats, not formal inferential guarantees; no multiplicity correction was applied. Because `test_public` participated in iterative screening and selection, it is not an independent final holdout. The independent official result remains `PRIVATE-NO-GO`, so these artifacts do not establish private generalization or production model quality.
