...
export const queryKeys = { jobs: ["jobs"] as const, job: (id: string) => ["jobs", id] as const, reviews: ["reviews"] as const, models: ["models"] as const, evidence: ["evidence"] as const, systemStatus: ["system-status"] as const };
...
export function useSystemStatus() { return useQuery({ queryKey: queryKeys.systemStatus, queryFn: ({ signal }) => api.getSystemStatus(signal), refetchInterval: 10_000 }); }
