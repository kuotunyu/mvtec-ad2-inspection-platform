...
function JobPlaceholder() {
  ...
  const count = (location.state as { queuedCount?: number } | null)?.queuedCount;
  return <div className="page"><header className="page-header"><div><span className="eyebrow">Job detail</span><h1>Inspection evidence</h1></div></header><EmptyState title={count ? `${count} images queued` : "Awaiting job evidence"}>Progress and per-image evidence will update here.</EmptyState></div>;
}
...
export function App() {
  return <AppShell><Routes>
    ...
    <Route path="/jobs/:jobId" element={<JobPlaceholder />} />
