...
export function UploadDropzone({ files, maxUploadLabel, onChange }: { files: File[]; maxUploadLabel: string; onChange: (files: File[]) => void }) {
  ...
  return <div className={`dropzone ${dragging ? "dropzone--active" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); onChange([...files, ...event.dataTransfer.files]); }}>
    <span aria-hidden="true" className="drop-icon"><span /></span><strong>將檢測影像拖曳至此</strong><p>PNG、JPEG 或 WebP · 單檔上限 {maxUploadLabel}</p>
